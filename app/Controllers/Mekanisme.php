...
use App\Models\M_Folder_Mek;
use App\Models\M_Mekanisme;
...
class Mekanisme extends BaseController
{
    ...
    {
        $this->folderModel = new M_Folder_Mek();
        $this->fileModel = new M_Mekanisme();
    }

    public function Mekanisme($id = null)
    {
    ...
    if ($id) {
        ...
        $files = $this->fileModel
            ->groupStart()
            ->where('id_folder_mek', null)
            ->orWhere('id_folder_mek', '')
            ->groupEnd()
            ->orderBy('created', 'DESC')
            ->findAll();
    }
    ...
    $data = [
        ...
        'current_folder' => $currentFolder ? $currentFolder['nama_folder_mek'] : 'Mekanisme',
        'folders' => $folders,
        'files' => $files,
        'id_parent' => $id,
        'asal' => 'mekanisme'
        ...
        echo view('Backend/Main/Mekanisme/mekanisme', $data);
    ...
    {
        $modelFolder = new M_Folder_Mek;
        $hasil = $modelFolder->autoNumber()->getRowArray();
        if (!$hasil) {
            $id = "FLRMK00001";
        } else {
            $kode = $hasil['id_folder_mek'];
            $noUrut = (int)substr($kode, -3);
            $noUrut++;
            $id = "FLRMK" . sprintf("%05s", $noUrut);
        }
        ...
        $dataUpdate = [
            'id_folder_mek'   => $id,
            'nama_folder_mek' => $name,
            'id_parent' => $parentId, // ubah dari parent_id
            'pemilik' => $pemilik,
            'created' => date('Y-m-d H:i:s')
        ];
        $modelFolder->saveDataFolderMek($dataUpdate);

        return redirect()->to('/admin/mekanisme/' . ($parentId ?? ''));
    }
...
{
    $modelFolder = new M_Mekanisme();
    $hasil = $modelFolder->autoNumber()->getRowArray();

    if (!$hasil) {
        $noUrut = 1;
    } else {
        $kode = $hasil['id_mekanisme'];
        ...
        if ($file->isValid() && !$file->hasMoved()) {
            ...
            try {
                ...
                $data = [
                    'id_mekanisme' => $id,
                    'nama_file'      => $fileName,
                    'id_folder_mek'  => $this->request->getPost('folder_id'),
                    'pemilik'        => $this->request->getPost('pemilik'),
                    'link'           => $fileUrl,
                    'created'        => date('Y-m-d H:i:s')
                ];
                $modelFolder->saveDataMekanisme($data);
    ...
    {
        $model = new \App\Models\M_Mekanisme();
        $drive = new \App\Libraries\GoogleDrive();

        // Ambil data hukum berdasarkan hash
        $data = $model->where('SHA1(id_mekanisme)', $idHash)->first();
        ...
        if ($data) {
            ...
            // Hapus dari database
            $model->where('id_mekanisme', $data['id_mekanisme'])->delete();
...
public function search()
{
    ...
    $fileModel = new \App\Models\M_Mekanisme();
    $folderModel = new \App\Models\M_Folder_Mek();
    ...
    $folders = $folderModel->like('nama_folder_mek', $keyword)
                           ->orLike('pemilik', $keyword)
                           ->findAll();
    ...
    public function hapus_folder($hash_id)
{
    ...
    $folder = $folderModel->where('SHA1(id_folder_mek)', $hash_id)->first();

    if (!$folder) {
        return redirect()->back()->with('error', 'Folder tidak ditemukan.');
    }

    $id = $folder['id_folder_mek'];
    ...
    $files = $fileModel->where('id_folder_mek', $id)->countAllResults();
    ...
    $folderModel->where('id_folder_mek', $id)->delete();

    return redirect()->back()->with('success', 'Folder berhasil dihapus.');
}
public function deleteSelectedFiles()
{
    $ids = $this->request->getPost('ids'); // array id_mekanisme asli
    if (!$ids || !is_array($ids)) {
    ...
    foreach ($ids as $id) {
        $data = $model->where('id_mekanisme', $id)->first();
        if ($data) {
            // 🔹 Ambil file_id dari URL Google Drive
            ...
            // 🔹 Hapus dari database
            $model->delete($data['id_mekanisme']);
            $deletedCount++;
        }
    }
...
public function addLink()
{
    ...
    $data = [
        'id_mekanisme' => $id,
        'nama_file'      => $nama,
        'id_folder_mek'  => $folderId,
        'pemilik'        => $pemilik,
        'link'           => $link,
        'created'        => date('Y-m-d H:i:s')
    ];

    $model->saveDataMekanisme($data);
...
public function editItem()
{
    ...
    if ($tipe === 'folder') {
        $model = new \App\Models\M_Folder_Mek();
        $update = $model->where('id_folder_mek', $id)->set([
            'nama_folder_mek' => $nama,
            'created' => $now
        ])->update();
    } else {
        $model = new \App\Models\M_Mekanisme();
        $update = $model->where('id_mekanisme', $id)->set([
            'nama_file' => $nama,
            'created' => $now
        ])->update();
    }
