  ...
  <nav aria-label="breadcrumb">
    <ol class="breadcrumb shadow-sm p-2 rounded">
      <li class="breadcrumb-item">
        <a href="<?= base_url('admin/perencanaan'); ?>">Perencanaan</a>
      </li>
      <?php foreach ($breadcrumb as $crumb): ?>
        <li class="breadcrumb-item">
          <a
            href="<?= base_url('admin/perencanaan/' . $crumb['id_folder_per']); ?>"><?= esc($crumb['nama_folder_per']); ?></a>
        </li>
      <?php endforeach; ?>
    </ol>
  </nav>
            ...
            <?php foreach ($folders as $folder): ?>
              <tr>
                ...
                <td>
                  ...
                  <a href="<?= base_url('admin/perencanaan/' . $folder['id_folder_per']); ?>"
                    class="ms-2 text-decoration-none"><?= esc($folder['nama_folder_per']); ?></a>
                </td>
                <td><?= esc($folder['pemilik']); ?></td>
                <td><?= esc($folder['created']); ?></td>
                <td>
                  <button type="button" class="btn btn-sm btn-warning btnEditFolder"
                    data-id="<?= $folder['id_folder_per']; ?>" data-nama="<?= esc($folder['nama_folder_per']); ?>">
                    <i class="bi bi-pencil"></i> Edit
                  </button>
<button type="button" class="btn btn-sm btn-secondary btnMoveItem"
  data-id="<?= $folder['id_folder_per']; ?>" data-tipe="folder">
  <i class="bi bi-folder-symlink"></i> Pindah
</button>

                  <a href="<?= base_url('/admin/perencanaan/hapus_folder/' . sha1($folder['id_folder_per'])); ?>"
            ...
            <?php foreach ($files as $file): ?>
              <tr>
                <td><?= $no++; ?></td>
                <td><input type="checkbox" name="selected_files[]" value="<?= esc($file['id_perencanaan']); ?>"></td>
                <td>
                ...
                <td>
                  <button type="button" class="btn btn-sm btn-warning btnEditFile"
                    data-id="<?= $file['id_perencanaan']; ?>" data-nama="<?= esc($file['nama_file']); ?>">
                    <i class="bi bi-pencil"></i> Edit
                  </button>
<button type="button" class="btn btn-sm btn-secondary btnMoveItem"
  data-id="<?= $file['id_perencanaan']; ?>" data-tipe="file">
  <i class="bi bi-folder-symlink"></i> Pindah
</button>

                  <a href="<?= base_url('/admin/perencanaan/hapus/' . sha1($file['id_perencanaan'])); ?>"
                    onclick="return confirm('Yakin ingin menghapus data ini?')" class="btn btn-sm btn-danger">
                    Hapus
                  </a>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title"><i class="bi bi-folder-symlink"></i> Pindah File / Folder</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
      </div>
<form id="formMoveItem" method="POST" action="<?= base_url('admin/perencanaan/pindah'); ?>">
  <div class="modal-body">
    <input type="hidden" name="id" id="move_id">
    <input type="hidden" name="tipe" id="move_tipe">

    <div class="mb-3">
      <label class="form-label">Pilih Folder Tujuan</label>
      <select name="tujuan" id="folder_tujuan" class="form-control" required>
        <option value="">-- Pilih Folder --</option>
        <?php foreach ($folders as $f): ?>
          <option value="<?= $f['id_folder_per']; ?>"><?= esc($f['nama_folder_per']); ?></option>
        <?php endforeach; ?>
      </select>
    </div>
  </div>
  <div class="modal-footer">
    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
    <button type="submit" class="btn btn-primary"><i class="bi bi-arrow-right-circle"></i> Pindahkan</button>
  </div>
</form>
...
<!-- Modal Tambah Banyak Folder -->
<div class="modal fade" id="addFolderModal" tabindex="-1">
  <div class="modal-dialog">
    <form id="multiFolderForm" method="post">
      <div class="modal-content">
        <div class="modal-header bg-success text-white">
          <h5 class="modal-title">Tambah Beberapa Folder</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
        </div>
        ...
        <div class="modal-body">
          ...
          <div id="folderContainer">
            <div class="mb-3 folder-item">
              ...
              <input type="text" name="nama[]" class="form-control" required placeholder="Masukkan nama folder">
            </div>
          </div>

          <button type="button" id="addFolderInput" class="btn btn-outline-success btn-sm">
            <i class="bi bi-plus-circle"></i> Tambah Folder Lagi
          </button>
        </div>
...
$('#formMoveItem').on('submit', function(e) {
  e.preventDefault();
...
<script>
  $('#addLinkForm').on('submit', function (e) {
    ...
    $.ajax({
      url: "<?= base_url('admin/perencanaan/addLink'); ?>",
      type: "POST",
      data: $(this).serialize(),
      dataType: "json",
      beforeSend: function () {
...
<script>
  ...
  $('#btnDownloadSelected').on('click', function () {
    ...
    $.ajax({
      url: "<?= base_url('admin/perencanaan/downloadSelected'); ?>",
      type: "POST",
      data: {
  ...
  $('#btnDeleteSelected').on('click', function () {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        $.ajax({
          url: "<?= base_url('admin/perencanaan/deleteSelectedFiles'); ?>",
          type: "POST",
          dataType: "json",
          data: {
            ids: selected
  ...
  async function uploadSequential(files) {
    ...
    `;
      ...
      await new Promise((resolve) => {
        ...
        xhr.open('POST', '<?= base_url('admin/perencanaan/addFile'); ?>', true);
...
<script>
  $(document).ready(function () {
    $('#globalSearch').on('keyup', function () {
      ...
      $.ajax({
        url: '<?= base_url('admin/perencanaan/search'); ?>',
        method: 'POST',
        data: {
          keyword: keyword
        },
          ...
          res.folders.forEach(folder => {
            ...
            <tr>
              <td><i class="bi-folder-fill text-warning"></i> 
                  <a href="<?= base_url('admin/perencanaan/'); ?>${folder.id_folder_per}" 
                     class="ms-2 text-decoration-none">${folder.nama_folder_per}</a></td>
...
<script>
  $(document).ready(function () {
    ...
    $('#editForm').on('submit', function (e) {
      ...
      $.ajax({
        url: "<?= base_url('admin/perencanaan/editItem'); ?>",
        type: "POST",
        data: $(this).serialize(),
        dataType: "json",
        beforeSend: function () {
...
<script>
  // Tambah input folder baru
  $('#addFolderInput').on('click', function() {
    $('#folderContainer').append(`
      <div class="mb-3 folder-item">
        <label>Nama Folder</label>
        <input type="text" name="nama[]" class="form-control mt-1" required placeholder="Masukkan nama folder">
      </div>
    `);
  });

  // Kirim data via AJAX
  $('#multiFolderForm').on('submit', function(e) {
    e.preventDefault();

    $.ajax({
      url: "<?= base_url('admin/perencanaan/addMultipleFolders'); ?>",
      type: "POST",
      data: $(this).serialize(),
      dataType: "json",
      beforeSend: function() {
        Swal.fire({
          title: 'Menyimpan...',
          text: 'Sedang membuat folder, mohon tunggu.',
          allowOutsideClick: false,
          didOpen: () => Swal.showLoading()
        });
      },
