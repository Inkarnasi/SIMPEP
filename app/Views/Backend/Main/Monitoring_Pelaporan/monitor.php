  ...
  <nav aria-label="breadcrumb">
    <ol class="breadcrumb shadow-sm p-2 rounded">
      <li class="breadcrumb-item">
        <a href="<?= base_url('admin/monitoring'); ?>">Monitoring Pelaporan</a>
      </li>
      <?php foreach ($breadcrumb as $crumb): ?>
        <li class="breadcrumb-item">
          <a
            href="<?= base_url('admin/monitoring/' . $crumb['id_folder_mon']); ?>"><?= esc($crumb['nama_folder_mon']); ?></a>
        </li>
      <?php endforeach; ?>
    </ol>
  </nav>
        ...
        <table id="fileTable" class="table table-striped">
          ...
          <tbody>
            <?php foreach ($folders as $folder): ?>
              <tr>
                ...
                <td>
                  ...
                  <a href="<?= base_url('admin/monitoring/' . $folder['id_folder_mon']); ?>"
                    class="ms-2 text-decoration-none"><?= esc($folder['nama_folder_mon']); ?></a>
                </td>
                <td><?= esc($folder['pemilik']); ?></td>
                <td><?= esc($folder['created']); ?></td>
                <td>
                  <button type="button" class="btn btn-sm btn-warning btnEditFolder"
                    data-id="<?= $folder['id_folder_mon']; ?>" data-nama="<?= esc($folder['nama_folder_mon']); ?>">
                    <i class="bi bi-pencil"></i> Edit
                  </button>
                  <a href="<?= base_url('/admin/monitoring/hapus_folder/' . sha1($folder['id_folder_mon'])); ?>"
            ...
            <?php foreach ($files as $file): ?>
              <tr>
                <td><input type="checkbox" name="selected_files[]" value="<?= esc($file['id_monitoring']); ?>"></td>
                <td><?= $no++; ?></td>
                <td>
                ...
                <td>
                  <button type="button" class="btn btn-sm btn-warning btnEditFile"
                    data-id="<?= $file['id_monitoring']; ?>" data-nama="<?= esc($file['nama_file']); ?>">
                    <i class="bi bi-pencil"></i> Edit
                  </button>
                  <a href="<?= base_url('/admin/monitoring/hapus/' . sha1($file['id_monitoring'])); ?>"
                    onclick="return confirm('Yakin ingin menghapus data ini?')" class="btn btn-sm btn-danger">
                    Hapus
                  </a>
...
<div class="modal fade" id="addFolderModal" tabindex="-1">
  <div class="modal-dialog">
    <form method="post" action="<?= base_url('admin/monitoring/addFolder'); ?>">
...
<script>
  $('#addLinkForm').on('submit', function (e) {
    ...
    $.ajax({
      url: "<?= base_url('admin/monitoring/addLink'); ?>",
      type: "POST",
      data: $(this).serialize(),
      dataType: "json",
      beforeSend: function () {
...
<script>
  ...
  $('#btnDownloadSelected').on('click', function () {
    ...
    $.ajax({
      url: "<?= base_url('admin/monitoring/downloadSelected'); ?>",
      type: "POST",
      data: {
  ...
  $('#btnDeleteSelected').on('click', function () {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        $.ajax({
          url: "<?= base_url('admin/monitoring/deleteSelectedFiles'); ?>",
          type: "POST",
          dataType: "json",
          data: {
            ids: selected
  ...
  async function uploadSequential(files) {
    ...
    `;
      ...
      await new Promise((resolve) => {
        ...
        xhr.open('POST', '<?= base_url('admin/monitoring/addFile'); ?>', true);
...
<script>
  $(document).ready(function () {
    $('#globalSearch').on('keyup', function () {
      ...
      $.ajax({
        url: '<?= base_url('admin/monitoring/search'); ?>',
        method: 'POST',
        data: {
          keyword: keyword
        },
          ...
          res.folders.forEach(folder => {
            ...
            <tr>
              <td><i class="bi-folder-fill text-warning"></i> 
                  <a href="<?= base_url('admin/monitoring/'); ?>${folder.id_folder_mon}" 
                     class="ms-2 text-decoration-none">${folder.nama_folder_mon}</a></td>
...
<script>
  $(document).ready(function () {
    ...
    $('#editForm').on('submit', function (e) {
      ...
      $.ajax({
        url: "<?= base_url('admin/monitoring/editItem'); ?>",
        type: "POST",
        data: $(this).serialize(),
        dataType: "json",
        beforeSend: function () {
